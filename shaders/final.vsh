...
flat out int main_mip_level;
flat out int hand_mip_level;
...
void main() {
    ...
    main_mip_level = findLSB(int(viewHeight / (isEyeInWater ? RESOLUTION_TARGET_MAIN_UNDERWATER : RESOLUTION_TARGET_MAIN)));
    hand_mip_level = findLSB(int(viewHeight / (isEyeInWater ? RESOLUTION_TARGET_HAND_UNDERWATER : RESOLUTION_TARGET_HAND)));
}